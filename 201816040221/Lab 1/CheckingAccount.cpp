...
CheckingAccount::CheckingAccount(double balan,double fee)
:Account( balan)
{
    transactionFee=fee;
}
...
bool CheckingAccount::debit(double amount)
{
    if(Account::debit(amount))
        if(Account::getBalance()-transactionFee>=0)
        chargeFee();
    return true;
    return false;
}
...
{
    cout<<"subtract transactionFee from the current balance"<<endl;
