...
SavingsAccount::SavingsAccount( double initialBalance, double iinterestRate )
   :Account(initialBalance)
{
   if ( iinterestRate >= 0.0  )
   {
        interestRate = iinterestRate;
   }
   else // otherwise, output message and set balance to 0.0
   {
      cout << "Error: Initial balance and interestRate cannot be negative." << endl;
      interestRate = 0.0;
   } // end if...else
